...
'Verify Search icon is present Cause Catagory page'
WebUI.verifyElementPresent(findTestObject('ProjectSetup_Page/Button_SearchIconCC'), 5)

'Verify Refresh icon is present Cause Catagory page'
WebUI.verifyElementPresent(findTestObject('ProjectSetup_Page/Btn_RefreshCC'), 5)

'Verify Add button is present Cause Catagory page'
WebUI.verifyElementPresent(findTestObject('ProjectSetup_Page/btn_AddCC'), 5)

'Verify Cause Category and Status fileds error message and add new Cause Category'
CustomKeywords.'apqpModule.ProjectSetup.addCauseCategory'('random')

'Verify whether newly created Cause Category is displayed correctly'
CustomKeywords.'apqpModule.ProjectSetup.verifyCauseCategory'()

'Delete the created Cause Category for cleanup'
CustomKeywords.'apqpModule.ProjectSetup.deleteCauseCategory'()
