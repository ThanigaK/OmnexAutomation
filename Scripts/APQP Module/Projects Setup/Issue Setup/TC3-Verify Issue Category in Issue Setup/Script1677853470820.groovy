...
WebUI.click(findTestObject('ProjectSetup_Page/lbl_Issue Category'))

'Verify Search icon is present issue category page'
WebUI.verifyElementPresent(findTestObject('ProjectSetup_Page/Button_SearchIconCC'), 5)

'Verify Add button is present issue category page'
WebUI.verifyElementPresent(findTestObject('ProjectSetup_Page/Button_AddSC'), 5)

'Verify issue category and Status fileds error message and add new issue category'
CustomKeywords.'apqpModule.ProjectSetup.addIssueCategory'('random')

'Click on the Add button'
WebUI.click(findTestObject('ProjectSetup_Page/Button_AddSC'))

'Create Random Name for Issue Category 2'
String IssueCategory = 'Issue_' + CustomKeywords.'apqpModule.Roles.RandomNumber'()

'Enter value on the text box below the Issue Category'
WebUI.sendKeys(findTestObject('ProjectSetup_Page/IssueCategoryAdd_TextBox'), IssueCategory)
...
CustomKeywords.'apqpModule.ProjectSetup.VerifyIssueCategory'()

CustomKeywords.'apqpModule.ProjectSetup.DeleteIssueCategory'()

'Click on Refresh icon in Issue Category page'
WebUI.click(findTestObject('ProjectSetup_Page/Btn_RefreshIC'), FailureHandling.STOP_ON_FAILURE)

CustomKeywords.'apqpModule.ProjectSetup.DeleteIssueCategory'(IssueCategory)
