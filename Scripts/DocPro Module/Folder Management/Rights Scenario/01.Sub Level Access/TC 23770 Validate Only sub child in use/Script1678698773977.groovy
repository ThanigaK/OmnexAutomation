...
'Right Click on the newly created Level'
WebUI.click(findTestObject('DocPro_Module/Folder Management/SearchedFolder_1'))
...
'Right Click on the newly created Level'
WebUI.rightClick(findTestObject('DocPro_Module/Folder Management/New_SubChild'))
...
for (int i = 1; i <= Column1.size(); i++) {
    ...
    if (AttachRight.contains('Attach Right')) {
        ...
        DriverFactory.getWebDriver().findElement(By.xpath('//input[@type=\'search\']')).sendKeys('Rights')

        WebUI.delay(3)

        DriverFactory.getWebDriver().findElement(By.xpath('(//input[@type=\'checkbox\' and contains(@id,\'Inherit\') and @checked=\'checked\'])[1]')).click()
